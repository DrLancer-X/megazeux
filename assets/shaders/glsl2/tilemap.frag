...
void main( void )
{
    ...
    int char_x = int(mod(float(char_num), 128.0)) * 8 + px;
    int char_y = char_num / 128 * 14 + py;
    
    bool pixset = texture2D(baseMap, vec2(1.0 / 1024.0 * float(char_x), 1.0 / 1024.0 * float(char_y))).x > 0.5;
    
    if(pixset)
    {
      gl_FragColor = texture2D( baseMap, vec2(1.0 / 1024.0 * float(fg_color), 448.0/1024.0));
    }
    else
    {
      gl_FragColor = texture2D( baseMap, vec2(1.0 / 1024.0 * float(bg_color), 448.0/1024.0));
    }
}
